...
def main():
    ...
    # 재 다운로드 및 MD5 검증 수행
    updated_success_files, updated_fail_files = update_call_md5(xml_file_names)
    print(f"업데이트된 성공한 XML 파일들 : {updated_success_files}")
    print(f"업데이트된 실패한 XML 파일들 : {updated_fail_files}")
...
def update_call_md5(xml_file_names):
    # 파일 경로 설정
    folder_path = "save_files"
    updated_success_xml_files = []
    updated_fail_xml_files = []
    ...
    for xml_file_name in xml_file_names:
        ...
        # MD5 검증 수행
        if verify_md5(file_path, md5_file_path):
            updated_success_xml_files.append(xml_file_name)
        else:
            updated_fail_xml_files.append(xml_file_name)
    
    return updated_success_xml_files, updated_fail_xml_files
